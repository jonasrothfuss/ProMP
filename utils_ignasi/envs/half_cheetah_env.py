...
class HalfCheetahEnv(MujocoEnv, Serializable):
    ...
    def __init__(self, task=None, reset_every_episode=False, reward=True, *args, **kwargs):
        ...
        #print("\n\nTASK RIGHT NOW: ", task)
        if task in ['None', None, 'forwback', 'mass', 'gravity', 'cripple', 'test_cripple', 'LegNothing']:
            self.task = task
            self.sign = 1
    ...
    def reset_task(self, value=None):
        ...
        #set one of the 6 joints to be crippled (and color it red)
        ...
        elif self.task == 'test_cripple':
            #set 1 joint to be crippled
                #this joint will not be functional in step func
            crippled_joint = 0 # value if value is not None else np.random.randint(1, self.action_dim)
            self.cripple_mask = np.ones(self.action_space.shape)
            self.cripple_mask[crippled_joint] = 0
            ...
            #set 1 joint to be crippled
                #this joint will not be functional in step func
            crippled_joint = 4 #set this joint to always 0
            self.cripple_mask = np.ones(self.action_space.shape)
            self.cripple_mask[crippled_joint] = 0
        ...
        elif self.task is None or self.task == 'None':
            pass
